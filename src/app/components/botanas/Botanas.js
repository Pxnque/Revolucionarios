 ...
 async function getPlatillos() {
    const url = `http://127.0.0.1:8090/api/collections/comida/records?filter=idCategoria='lsw804m2pecp6k6'&page=1&perPage=30`;
...
export default async function DisplayBotanas() {
    ...
    return (
        <div>
            <div className="flex items-center justify-center flex-wrap max-w-screen-xl">
                {platillos?.map((cat) => {
                        return <Botanas key={cat.nombre} note={cat}/>;
                })}
            </div>
        </div>
    );
