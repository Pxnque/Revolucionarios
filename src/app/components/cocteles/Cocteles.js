 ...
 async function getPlatillos() {
    const url = `http://127.0.0.1:8090/api/collections/comida/records?filter=idCategoria='n9oouzl7u6xr76e'&page=1&perPage=30`;
...
export default async function DisplayCocteles() {
    ...
    return (
        <div>
            <div className="flex items-center justify-center flex-wrap max-w-screen-xl">
                {platillos?.map((cat) => {
                        return <Cocteles key={cat.nombre} note={cat}/>;
                })}
            </div>
        </div>
    );
